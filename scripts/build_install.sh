...
#
# Usage:
#   ./scripts/build_install.sh [--no-sudo] [--arrow-prefix DIR] [--arrow-version V]
#
# Options:
...
if [ "$USE_SYSTEM_ARROW" = "1" ] && [ "$BUILD_ARROW" = "0" ]; then
  ...
  if ! ${SUDO} ${APT} install -y libarrow-dev libparquet-dev libarrow-compute-dev libarrow-dataset-dev; then
    echo "[!] Arrow APT install failed, falling back to source build..." >&2
    BUILD_ARROW=1
    USE_SYSTEM_ARROW=0
  fi
fi
