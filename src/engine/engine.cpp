...
template <typename TYPE,
          typename = typename std::enable_if<arrow::is_number_type<TYPE>::value | arrow::is_boolean_type<TYPE>::value | arrow::is_temporal_type<TYPE>::value>::type>
...
arrow::Result<std::shared_ptr<arrow::Table>> GetTable() {
  ...
  auto record_batch = arrow::RecordBatch::Make(arrow::schema({ arrow::field("a", arrow::int64()),
...
arrow::Status SourceScalarAggregateSinkExample() {
  ...
  cp::Declaration source{ "source", std::move(source_node_options) };
  auto aggregate_options = cp::AggregateNodeOptions{ /*aggregates=*/{ { "sum", nullptr, "a", "sum(a)" } } };
...
arrow::Status SourceGroupAggregateSinkExample() {
  ...
  auto aggregate_options = cp::AggregateNodeOptions{ /*aggregates=*/{ { "hash_count", options, "a", "count(a)" } },
                                                     /*keys=*/{ "b" } };
  cp::Declaration aggregate{
    "aggregate", { std::move(source) }, std::move(aggregate_options)
  };
...
arrow::Status ExecutePlanAndCollectAsTableWithCustomSink(
  ...
  std::shared_ptr<arrow::RecordBatchReader> sink_reader = cp::MakeGeneratorReader(schema, std::move(sink_gen), arrow::default_memory_pool());
...
arrow::Status RecordBatchReaderSourceSinkExample() {
  ARROW_ASSIGN_OR_RAISE(auto table, GetTable());
  std::shared_ptr<arrow::RecordBatchReader> reader = std::make_shared<arrow::TableBatchReader>(table);
...
const arrow::compute::Declaration& engine::getSource(Connector& con) {
  ...
  arrow::compute::Declaration source{ con.connector_name, std::move(table_source_options) };
  return source;
}
} // namespace engine
} // namespace surfingdb