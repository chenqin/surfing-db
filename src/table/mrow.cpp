...
mrow::~mrow() { _payload = nullptr; }
/**
 * @brief max buffer size from schema
 *
 * @return size_t
 */
size_t mrow::capacity() { return this->schema_ptr->rowSize(); }

uint8_t* mrow::payload_ptr() { return this->_payload; }
...
size_t mrow::write(const Field& f, const Value& v, const uint64_t& offset) {
  switch (f.type) {
    case matcha::meta::schema::RowType::VOID: {
      assert(false);
      return 0;
    }
    case matcha::meta::schema::RowType::BOOL: {
      _pwrite(f, &v.p_val.bool_val, offset);
      return sizeof(bool);
    }
    case matcha::meta::schema::RowType::CHAR: {
      _pwrite(f, &v.p_val.byte_val, offset);
      return sizeof(char);
    }
    case matcha::meta::schema::RowType::INT: {
    ...
    case RowType::STRING: {
      CHECK_LE(strlen(v.p_val.string_val.c_str()),
               MAX_STR_LEN - 1);  // include extra \0
    ...
    case RowType::LIST: {
      ...
      listField.max_unit_size =
          matcha::meta::SchemaUtils::getFieldSize(listField);
    ...
    case RowType::MAP: {
      ...
      keyField.max_unit_size =
          matcha::meta::SchemaUtils::getFieldSize(keyField);
      valueField.max_unit_size =
          matcha::meta::SchemaUtils::getFieldSize(valueField);
      ...
      for (auto& pair : v.map_value) {
        ...
        Value keyread;
        map_offset += matcha::meta::SchemaUtils::getFieldSize(keyField);
        mem_size += write(valueField, value, map_offset);
        map_offset += matcha::meta::SchemaUtils::getFieldSize(valueField);
      }
      return mem_size;
    }
  }
...
size_t mrow::_pread(const Field& f, void* dataptr, const uint64_t& offset) {
  switch (f.type) {
    ...
    case RowType::STRING: {
      char* char_ptr = (char*)(_payload + offset);
      size_t resid =
          MAX_STR_LEN - strlen(char_ptr) - 1;  // leave extra byte'\0'
      CHECK_GE(resid, 0);
