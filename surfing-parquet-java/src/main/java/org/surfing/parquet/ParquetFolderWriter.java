...
public class ParquetFolderWriter {
    ...
    public static void writeParquetFile(VectorSchemaRoot root, String filePath, Configuration conf) throws IOException {
        ...
        org.apache.parquet.schema.MessageType parquetSchema =
            org.apache.parquet.arrow.schema.SchemaConverter.fromArrow(arrowSchema).getParquetSchema();

        // Set up writer properties
        org.apache.parquet.hadoop.ParquetWriter.Builder<org.apache.arrow.vector.VectorSchemaRoot> builder =
            org.apache.parquet.arrow.ArrowParquetWriter.builder(outputFile)
                .withSchema(arrowSchema)
                .withCompressionCodec(CompressionCodecName.SNAPPY)
                .withConf(conf);

        try (ParquetWriter<VectorSchemaRoot> writer = builder.build()) {
            writer.write(root);
        } catch (Exception e) {
